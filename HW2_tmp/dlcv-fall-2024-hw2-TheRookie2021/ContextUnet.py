""" from https://github.com/byrkbrk/conditional-ddpm/blob/main/models.py """
import torch
...
class ContextUnet(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        for up_block, down, contextemb, timeemb in zip(self.up_blocks, downs[::-1], self.contextembs, self.timeembs):
            # print(f"{t.dtype=}, {c.dtype=}")
            # print(f"{t.shape=}, {c.shape=}")
            up = up_block(up*contextemb(c) + timeemb(t), down)
        out= self.final_conv(torch.cat([up, x], axis=1))
        return out
    
if __name__ == '__main__':
    model = ContextUnet(in_channels=3, height=32, width=32, n_feat=64, n_cfeat=10, n_downs=4)

    img = torch.randn((64, 3, 32, 32))
    t = torch.randint(1, 10 + 1, (64, ))    
    c = torch.randint(0, 10 , (64, ))  

    c_one_hot=torch.nn.functional.one_hot(c, num_classes=10)
    print(c.shape)
    print(c_one_hot.shape)
    print(c,c_one_hot)
    # print(f"{img.shape=}")
    # print(f"{c.shape=}")
    # print(f"{get_masked_context(c).shape=}")
    # print(f"{t.shape=}")

    # # print(model)
    # out = model(img, t, get_masked_context(c))
